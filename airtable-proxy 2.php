...
switch ($action) {
    ...
    case 'login':
        ...
        // Check admin temp code first
        $adminTmp = sys_get_temp_dir() . '/nc_admin_code.txt';
        if (file_exists($adminTmp)) {
            $parts = explode(':', trim(file_get_contents($adminTmp)));
            if (count($parts) === 2 && $parts[0] === $code && (time() - (int)$parts[1]) < 1800) {
                unlink($adminTmp);
                echo json_encode(array('ok' => true, 'isAdmin' => true));
                break;
            }
        }
